...
export class LoginComponent implements OnInit {
  ...
  onLogin() {
    ...
    this.auth.loginUser(employee).subscribe(
      data => {
        this.loading = false;
        ...
        if (data.employee) {
          ...
          this.router
            .navigate(["/"])
            .then(() => {
              this.message.show(`¡Bienvenido ${this.username}!`, {
          ...
          this.router
            .navigate(["/login"])
            .then(() => {
              this.message.show(`Error con el servidor`, {
                cssClass: "alert-danger",
                timeout: 5000
              });
            })
      ...
      err => {
        this.loading = false;
        this.message.show("Usuario o contraseña incorrecto", {
          cssClass: "alert-danger",
          timeout: 15000
        });
      }
